...
courier_experience_yrs = st.number_input(
    "Courier Experience (years)", min_value=0.0, max_value=20.0, value=2.0, step=0.1
)
...
if st.button("Predict Delivery Time"):
    ...
    traffic_level_encoded = label_encoders["Traffic_Level"].transform(
        [traffic_level_selected]
    )[0]
    time_of_day_encoded = label_encoders["Time_of_Day"].transform(
        [time_of_day_selected]
    )[0]
    vehicle_type_encoded = label_encoders["Vehicle_Type"].transform(
